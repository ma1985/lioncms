...
class Collect extends Base
{
    ...
    private function syncImages($pic_status, $pic_url, $flag = 'vod')
    {
        ...
        if ($pic_status == 1) {
            ...
            $pics = model('Image')->down_load($pic_url, $config, $flag);
            $img_url_downloaded = $pics['pic'];
        ...
        return ['pic' => $img_url_downloaded, 'pic_thumb' => $pics['pic_thumb'], 'msg' => $des];
    }
            ...
            if ($v['type_id'] == 0) {
                ...
                if (!$info) {
                    ...
                    if ($param['opt'] == 2) {
                        ...
                        $v['vod_pic_slide'] = (string)$tmp['pic'];
                        $v['vod_pic_thumb'] = (string)$tmp['pic_thumb'];
                        $msg = $tmp['msg'];
                    ...
                    if (empty($config['uprule'])) {
                        ...
                        if (strpos(',' . $config['uprule'], 'j') !== false && (substr($info["vod_pic"], 0, 4) == "http" || empty($info['vod_pic'])) && $v['vod_pic'] != $info['vod_pic']) {
                            $tmp = $this->syncImages($config_sync_pic, $v['vod_pic'], 'vod');
                            $update['vod_pic_slide'] = (string)$tmp['pic'];
                            $update['vod_pic_thumb'] = (string)$tmp['pic_thumb'];
                            $msg = $tmp['msg'];
                        }
