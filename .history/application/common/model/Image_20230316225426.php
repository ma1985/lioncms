...
class Image extends Base
{
    ...
    public function down_exec($url, $config, $flag = 'vod')
    {
        ...
        $uniqid_name = md5(uniqid());
        $file_name = $uniqid_name . '.' . $ext;
        // 上传附件路径
        ...
        // 缩略图
        if ($config['thumb'] == 1) {
            $thumb = $this->makethumb($_file_path, $n_dir, $uniqid_name, $config, $flag);
        }
        //上传到远程
    ...
    public function makethumb($file_path, $n_dir, $uniqid_name, $config, $flag = 'vod', $new = 1)
    {
        ...
        if (!empty($config['thumb_size'])) {
            try {
                ...
                $t_size = explode('x', strtolower($config['thumb_size']));
                if (!isset($t_size[1])) {
                    $t_size[1] = $t_size[0];
                }
                // $new_thumb = 'upload' . '/' . 'vodthumb' . '/' . $n_dir .  '/' . $file_name . '-' . $t_size[0] . 'x' . $t_size[1] . '.' . strtolower(pathinfo($file_path, PATHINFO_EXTENSION));



                $new_thumb  = 'vodthumb/' . $t_size[0] . 'x' . $t_size[1] . '/' . $n_dir .  '/' . $uniqid_name . '.' . strtolower(pathinfo($file_path, PATHINFO_EXTENSION));
                if ($new == 0) {
                    $new_thumb = $file_path;
                }
                $dir = dirname(ROOT_PATH . 'upload/' . $new_thumb);
                if (!is_dir($dir)) {
                    mac_mkdirss($dir);
                }
                $image->thumb($t_size[0], $t_size[1], $thumb_type)->save('./upload/' . $new_thumb);
                $thumb = $new_thumb;
                // $data['size'] = $t_size[0] . 'x' . $t_size[1];
                // $thumb_size = round(filesize('./upload/' . $new_thumb) / 1024, 2);

                // $data[$k]['type'] = 'image';
                // $data[$k]['flag'] = $flag;

                // $data[$k]['url'] = $new_thumb;
                // $data[$k]['size'] = $t_size[0] . 'x' . $t_size[1];
                // $data[$k]['ctime'] = request()->time();
