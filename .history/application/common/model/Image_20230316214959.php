...
class Image extends Base
{
    ...
    public function down_exec($url, $config, $flag = 'vod')
    {
        ...
        if ($config['thumb'] == 1) {
            ...
            // unshaken 拼接thumb 连接
            //$n_dir, $file_name, 
            //upload/vod/20230316-1/604154b1b3826e350e7ed111d94f9d9a.jpg

            // unshaken start
            // $_thumb_path  = 'upload' . '/' . $flag . 'thumb/'. $n_dir . '/'.$uniqid_name . '.' . $ext;

            // unshaken end
            $this->makethumb($_file_path, $n_dir, $uniqid_name, $config, $flag);
        }
        //上传到远程
        ...
        if (!empty($config['thumb_size'])) {
            try {
                ...
                foreach ($thumbs as $k => $v) {
                    ...
                    $new_thumb  = ROOT_PATH . 'public/' . 'upload' . '/' . $flag . 'thumb/' . $n_dir . '/' . $uniqid_name . '-' . $t_size[0] . 'x' . $t_size[1] . '.' . strtolower(pathinfo($file_path, PATHINFO_EXTENSION));
                    var_dump($new_thumb);
                    if ($new == 0) {
                        $new_thumb = $file_path;
                    }
